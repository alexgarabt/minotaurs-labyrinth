...
def main(filename: str, file_name_output: str, labyrinth: Labyrinth = Labyrinth(max_area=MAX_AREA)):
    """
    Reads a file with the description of various labyriths and resolve each labyrith 
    and write it in the output files
    """
    with open(file_name_output, 'w') as output:
        with open(filename, 'r') as file:
            ...
            while True:
                ...
                result = labyrinth.teseo_to_minotaurs()
                
                is_possible = result[0]
                num_doors = result[2]

                if is_possible: 
                    output.write(str(num_doors)+"\n")
                    print(str(num_doors)+"\n")
                else: 
                    output.write(ERROR_NUM+"\n")
                    print(ERROR_NUM+"\n")
...
if __name__ == '__main__':
    ...
    main("../data_files/entrada.txt", "../data_files/salida.txt", labyrinth)
